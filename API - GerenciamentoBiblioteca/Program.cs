...
app.MapGet("/api/livro/buscar/{id}", ([FromRoute] string Id, [FromServices] BibliotecaContext banco) =>
{
    Livro? livroBusca = banco.Livros.FirstOrDefault(u => u.Id == Id);

    var livros = banco.Livros.ToList();

    ArrayList LivroEncontrado = [];

    if(livroBusca != null){
        foreach (Livro taf in livros)
    {
        if (taf.Id == Id)
        {
            Livro? livro = banco.Livros.Find(taf.Id);

            LivroEncontrado.Add(taf);
        }
